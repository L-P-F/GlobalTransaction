 ...
 */
public class GTFeignReqSendInterceptor implements RequestInterceptor
{
    @Override
    public void apply(RequestTemplate template)
    {
        String xid = GTContext.getXid(); //只有通过GlobalTransaction注解的方法才有这个xid,如果xid为null,代表本次向外发送的feign请求不属于任何一个全局事务
        if (xid != null)
